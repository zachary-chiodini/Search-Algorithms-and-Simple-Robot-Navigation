<h1>Search Algorithms and Simple Robot Navigation</h1>
 ...
 <h3>*Please click on the README.md file and view the page in full screen or animations may not be in sync.* ♾️</h3>
...
<p style="text-align:justify">
  ...
  N robots provide N sets of 9 actions, which provides at most 9<sup>N</sup> 
...
<p style="text-align:justify">
  ...
  Child nodes represent the states reachable from the parent state via the successor set.
  When children of a node are generated by the successor, it is termed "expanding" the node.
  The action taken to reach each child state is represented by branches connecting the parent node to each child node. 
  The initial state is the root node, and the goal state is a leaf node.
  A solution is a path from the root node to the node containing the goal state.
...
<p style="text-align:justify">
  ...
  The optimal solution for this problem is the quickest solution, not necessarily the solution in which the robots travel the least distance.
